...
AlgoLab is a p5.js environment for creating and performing audio-reactive
...
AlgoLab satisfies **Degree 3 liveness**:
...
The existing **Live Strategy Audio Instrument** demonstrates safe runtime
...
defined. AlgoLab must additionally let performers author and replace those
...
AlgoLab should feel like a p5.js sketch that has learned how to stay alive
...
Syntax errors, evaluation errors, and first-frame runtime errors leave the
...
strategy state without being forced to read the entire development interface.
...
- provide starter strategies and examples;
- demonstrate live replacement in class;
- inspect a student's active code and strategy history;
...
AlgoLab—not student code—owns the p5.js `setup()` and `draw()` functions.
Student code supplies named behaviors that the host calls from its persistent
draw loop.

### Replace small units, not the entire sketch

The primary live-coded unit is a named **strategy**. Re-evaluating a strategy replaces
that one behavior. This makes the consequence of an edit easier to predict and
recover.

### The last good image-making system wins

New code is a candidate until it compiles, validates successfully, and survives
its first invocation. Failure restores the previous version.

### State has an identity

A strategy's code may change while its state persists. State belongs to the strategy
name, not to one compiled function body.

### Audio is shared infrastructure

p5.sound analysis is computed once per frame by the host. Strategies consume a
...
canvas and, optionally, a deliberate code or strategy overlay.
...
supply their own named strategy functions or objects.
...
function draw() {
  ...
  const drawInputs = runtime.beginFrame(audio);

  for (const strategy of activeScene.strategies) {
    runtime.drawStrategy(strategy, drawInputs);
  }
...
### 9.1 Minimum strategy form

Students define a named strategy as a function, an object with `draw()`, or a class
instance with `draw()`. A function is the minimum form:

```js
const wash = ({ audio }) => {
  background(audio.level * 40);
};
```

An object adds properties, helper methods, and `this`:
...
Evaluating a new `const rings = ...` block replaces the active `rings` behavior on
the next draw. No page reload occurs. A `// %%` marker groups a class or factory and
the strategy it constructs into one atomic live-coding cell.

### 9.2 Stateful strategy form

Strategies may declare state that survives later code replacements:

```js
const orbiters = {
  state() {
    return { angle: 0, trails: [] };
  },
...
- `state` initializes JSON-compatible strategy state once.
- `enter` runs when a scene activates the strategy.
- `draw` runs once per rendered frame.
- `beat` runs once on the rising edge of a detected onset.
- `exit` runs when the strategy leaves the active scene.

Only `draw` is required.

### 9.4 Draw inputs

Each handler receives one ordinary object and destructures the changing live inputs it
needs. Configuration belongs in function closures, object properties, or constructors;
ordinary methods accept ordinary student-supplied parameters.

| Property    | Meaning                                                        |
| ----------- | -------------------------------------------------------------- |
| `audio`     | Read-only audio snapshot for the current frame                 |
| `state`     | Persistent state owned by this strategy identity                  |
...
| `params`    | Live parameter values declared by the strategy                    |
| `controls`  | Read-only keyboard/MIDI state when available                   |

Global p5.js drawing functions remain available in strategy code. The host wraps
each strategy invocation in `push()` / `pop()` and restores required shared drawing
defaults to reduce accidental style leakage between strategies.
...
Named strategies are combined into named scenes:
...
there is no parallel add/remove command or UI-owned scene order to become inconsistent.

### 9.7 Live parameters

A strategy may declare a small number of performable values:
...
or controls. Parameter manipulation provides Degree 2 liveness; live strategy
...
| **Strategy shelf**     | Named strategies, active/inactive status, version, and error state |
...
| **Runtime monitor** | FPS, active strategy, evaluation status, last error                |
...
The editor determines blocks from top-level object/class declarations, scene arrays,
and command expressions. If block detection is uncertain, it selects the smallest
...
- its strategy receives a new visible version number;
...
- audio, time, and compatible strategy state continue.

### 10.5 Failed evaluation

On syntax, evaluation, or shape-validation failure:

- the candidate is not committed;
- the last valid strategy continues drawing;
...
- the user can dismiss the message without changing the active strategy.

On a strategy's first-frame runtime failure:

- the host catches the error around that strategy invocation;
- the candidate version is marked failed;
- the previous version is restored for the next frame;
- the strategy's pre-candidate JSON-compatible state snapshot is restored;
...
3. **Canvas + system trace:** visual output with strategy names, layer order, and
...
| R-02 | P0       | The host shall own `setup()` and `draw()`; student live code shall supply named strategy functions or objects rather than replace the host loop. |
| R-03 | P0       | Valid strategy replacements shall become active at the next frame boundary without a page reload.                                |
| R-04 | P0       | Strategies shall be able to call standard global p5.js drawing, math, color, noise, and geometry functions.                      |
| R-05 | P0       | Each strategy invocation shall be isolated with p5.js drawing-state protection where practical.                                  |
| R-06 | P1       | Canvas resize and fullscreen changes shall preserve strategy registrations and state.                                            |
...
| A-03 | P0       | FFT and amplitude analysis shall run once per frame regardless of the number of active strategies.                        |
| A-04 | P0       | Code evaluation shall not restart playback, reset playback position, or recreate the audio analyzer.                   |
| A-05 | P0       | All strategies in a frame shall receive the same read-only audio snapshot.                                                |
...
### Live code and strategies

| ID   | Priority | Requirement                                                                                                             |
| ---- | -------- | ----------------------------------------------------------------------------------------------------------------------- |
| L-01 | P0       | A performer shall be able to define, evaluate, and replace a named p5.js visual strategy while the sketch runs.            |
| L-02 | P0       | Evaluating one strategy shall not re-evaluate or reset unrelated strategies.                                                  |
| L-03 | P0       | Replacing a strategy by the same name shall preserve compatible strategy state.                                               |
| L-04 | P0       | A performer shall be able to explicitly reset one strategy's state.                                                        |
| L-05 | P0       | A performer shall be able to define and activate a named scene composed of multiple strategies.                            |
...
| ID   | Priority | Requirement                                                                                                            |
| ---- | -------- | ---------------------------------------------------------------------------------------------------------------------- |
| S-01 | P0       | A syntax error shall never replace a valid active strategy.                                                               |
| S-02 | P0       | An evaluation or shape-validation error shall never replace a valid active strategy.                                      |
| S-03 | P0       | A first-frame runtime error shall automatically restore the previous valid strategy version.                              |
| S-04 | P0       | One failing strategy shall not stop unrelated active strategies or the host draw loop.                                       |
| S-05 | P0       | The system shall retain at least ten successful versions per strategy for one-click reversion.                            |
...
| S-09 | P2       | The runtime shall detect likely runaway strategy execution in an isolated worker or frame boundary and offer termination. |
...
| P-03 | P1       | Projection view shall optionally show a readable strategy and mapping trace.           |
...
| ID   | Priority | Requirement                                                                                                        |
| ---- | -------- | ------------------------------------------------------------------------------------------------------------------ |
| D-01 | P0       | The current editor source, strategy registry, scenes, and declared parameters shall persist locally after refresh.    |
| D-02 | P1       | The user shall be able to export a human-readable project containing source, scene composition, and live parameter values. |
...
10. The performer reorders the scene's strategies and then reverts `rings` to an
...
- staging and active strategy registries;
- structured-clone-compatible state storage keyed by strategy instance id;
...
2. Execute it against captured prior bindings and a staging transaction.
3. Discover and validate strategy functions/objects, scene arrays, and live commands.
4. Snapshot compatible strategy state.
...
the constrained parser used in the earlier Live Strategy lesson.
...
Strategy state is expected to be composed of numbers, strings, booleans, arrays,
...
resources rather than persistent strategy state.

If a new strategy version expects a different state shape, the performer may:

- migrate it explicitly in the evaluated block;
- reset that strategy;
- bind the strategy under a new name.
...
- named object discovery and atomic strategy replacement;
- persistent per-strategy state;
- named scenes and live recomposition;
- syntax/evaluation rejection;
...
- WebGL and shader strategy lifecycle;
- collaborative rooms;
...
- At least 80% of pilot students can make their first successful live strategy
  replacement within 15 minutes using only the starter file and inline help.
- At least 80% can explain why editing a strategy does not replace `draw()` after
  one lab session.
- At least 75% can intentionally preserve, reset, and migrate strategy state in a
  guided exercise.
...
- A failing strategy does not stop other strategies in automated integration tests.
- Strategy history restores the corresponding source and behavior in all supported
...
Give students three starter strategies and ask them to:

- alter one behavior;
- create a new named strategy;
- combine two strategies into a scene;
...
| p5 drawing state leaks across strategies                   | Reordering strategies changes them unpredictably    | Wrap calls in `push()`/`pop()` and restore documented defaults                                |
...
| Students mistake presets for Degree 3                   | Only parameters change                           | Require authored strategy replacement in the acceptance test and show the active version history |
...
AlgoLab makes the course's advanced JavaScript topics structurally necessary:
...
| Higher-order functions | strategy factories, mappings, modifiers, and language adapters |
...
| Composition            | multiple strategies forming a live scene                       |
...
1. Is first-frame rollback sufficient, or should candidate strategies pass a
...
AlgoLab is ready for a course pilot when:
...
- the complete Degree 3 acceptance scenario passes in the supported classroom
  browser;
- a student unfamiliar with the system can replace a starter strategy within 15
  minutes;
